...
//
// Vendored into lymon-plugin-s7 from https://github.com/davenardella/rust7 (MIT) @ main.
// Reformatted with rustfmt; our changes are marked `// LYMON DELTA:`. To re-sync: re-vendor
// upstream client.rs, re-apply the marked deltas, and run rustfmt. MIT copyright retained above.
...
impl S7Client {
    ...
    pub fn read_area(
        ...
        while offset < datasize {
            ...
            // LYMON DELTA: read the FULL S7 telegram with read_exact, not a single
            // read() that can short-read under TCP segmentation and spuriously fail
            // on large PDUs (e.g. near the negotiated PDU size).
            ...
            // Copy payload (s7_comm_size = full S7 telegram length, read in full above)
            let payload = &response[READ_RES_LEN
                ..READ_RES_LEN + (s7_comm_size - READ_RES_LEN).min(chunk_size as usize)];
    ...
    // Writes a full telegram, reads the 7-byte TPKT+ISO header (read_exact), validates
    // it, then reads the S7 payload (read_exact). Returns the S7 PDU bytes (everything
    // after the 7-byte TPKT/ISO header). Reconnect-on-error stays the caller's job.
